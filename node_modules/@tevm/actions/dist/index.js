...
var handleRunTxError = (e) => {
  if (e instanceof Error) {
    ...
    if (e.message.includes("block has a different hardfork than the vm")) {
      return new MisconfiguredClientError(e.message, { cause: (
        /** @type {any}*/
        e
      ) });
    }
    ...
    if (e.message.includes("sender doesn't have enough funds to send tx.")) {
      return new InsufficientBalanceError(e.message, { cause: (
        /** @type {any}*/
        e
      ) });
    }
    return new InternalEvmError(e.message, { cause: (
      /** @type {any}*/
      e
    ) });
  }
  if (!(e instanceof EvmError)) {
    return new InternalEvmError("Unknown error", { cause: (
      /** @type {any}*/
      e
    ) });
  }
...
var executeCall = async (client, evmInput, params, events) => {
  ...
  try {
    ...
    if (params.createTrace) {
      trace = await runCallWithTrace(vm, client.logger, evmInput, true).then(({ trace: trace2 }) => trace2);
    }
    ...
    if (trace) {
      ...
      trace.failed = false;
      trace.returnValue = bytesToHex$1(
        /** @type {any} */
...
var handlePendingTransactionsWarning = async (client, params, code, deployedBytecode) => {
  if (code === void 0 && deployedBytecode === void 0 && params.to !== void 0 && params.data !== void 0 && hexToBytes(params.data).length > 0) {
    const vm = await client.getVm();
    const isCode = await vm.stateManager.getCode(createAddress(params.to)).then((code2) => code2.length > 0).catch(() => false);
    const txPool = await client.getTxPool();
...
var zBlockOverrideSet = z.strictObject({
...
var zStateOverrideSet = z.record(
  zAddress,
  z.strictObject({
...
var zBaseCallParams = zBaseParams.extend({
  ...
  createAccessList: z.boolean().optional().describe("If true, the call will also return a `accessList` on the accessList property"),
  ...
  gas: z.bigint().nonnegative().optional().describe("The gas limit for the call. Defaults to `16777215` (`0xffffff`)"),
  value: z.bigint().nonnegative().optional().describe("The value in ether that is being sent to `opts.address`. Defaults to `0`"),
  ...
  maxPriorityFeePerGas: z.bigint().optional().describe("The maximum priority fee per gas for the call for an EIP-1559 tx.")
}).refine(
  (params) => {
...
var zCallParams = z.intersection(
  zBaseCallParams,
  z.object({
...
var callHandler2 = (client, { throwOnFail: defaultThrowOnFail = true } = {}) => async ({ code, deployedBytecode, onStep, onNewContract, onBeforeMessage, onAfterMessage, ...params }) => {
  ...
  const l1FeeInfoPromise = vm.common.sourceId !== void 0 ? getL1FeeInformationOpStack(evmInput.data ?? numberToBytes(0), vm).catch((e) => {
...
var zAbi = Abi.describe("A valid ABI");

// src/Contract/zContractParams.js
var zContractParams = z.intersection(
  zBaseCallParams,
  z.object({
...
var callProcedure = (client) => async (request) => {
  const { errors = [], ...result } = await callHandler2(client)({
    throwOnFail: false,
    ...request.params[1] ? {
  ...
  if (errors.length > 0) {
    ...
    return {
      ...
      error: {
        ...
        data: {
          data: result.rawData,
          errors: errors.map(({ message: message2 }) => message2)
        }
      },
      method: "tevm_call",
...
var dealHandler = (node) => async ({ erc20, account, amount }) => {
  ...
  for (const { address, storageKeys } of accessListResponse.result.accessList) {
    for (const slot of storageKeys) {
      ...
      if (!balanceResponse.errors && balanceResponse.data === hexToBigInt$1(value)) {
...
var anvilDropTransactionJsonRpcProcedure = (client) => {
  return async (request) => {
    ...
    if (
      /** @type {any[]}*/
      txPool.getByHash([hexToBytes(txHash)]).length > 0
    ) {
...
var anvilResetJsonRpcProcedure = (node) => {
  return async (request) => {
    ...
    txs.forEach((tx) => {
      txPool.removeByHash(bytesToHex$1(tx.hash()));
    });
    ...
    receiptManager.mapDb = createMapDb({ cache: /* @__PURE__ */ new Map() });
    ...
    result.errors = result.errors.map((err) => {
      if (isHex(err.message) && err instanceof RevertError) {
        try {
          ...
          return new RevertError(message, { cause: (
            /** @type any*/
            err
          ) });
        } catch (e) {
  ...
  if (diffMode) {
    for (const address of preimages) {
      try {
        ...
        const slots = accessList.get(address) ?? /* @__PURE__ */ new Set();
...
var traceCallHandler = (client) => (
  ...
  (params) => {
    ...
    const callParams = {
      ...
      ...params.data !== void 0 ? { data: hexToBytes(params.data) } : {},
  ...
  return async (request) => {
    ...
    for (let i = 0; i < block.transactions.length; i++) {
      ...
      const traceResult = await traceCallHandler({ ...client, getVm: () => Promise.resolve(vmClone) })({
        ...
        ...txParams.gasLimit !== void 0 ? { gas: hexToBigInt(txParams.gasLimit) } : {},
        ...txParams.gasPrice !== void 0 ? { gasPrice: hexToBigInt(txParams.gasPrice) } : {},
        ...txParams.value !== void 0 ? { value: hexToBigInt(txParams.value) } : {},
...
var txToJsonRpcTx = (tx, block, txIndex) => {
  ...
  return (
    /** @type {import('../common/TransactionResult.js').TransactionResult} */
    {
      ...
      ...txJSON.value !== void 0 ? { value: typeof txJSON.value === "string" ? txJSON.value : numberToHex(txJSON.value) } : {},
      ...
      ...txJSON.chainId !== void 0 ? { chainId: typeof txJSON.chainId === "string" ? txJSON.chainId : numberToHex(txJSON.chainId) } : {},
...
var parseBlockParam = async (blockchain, blockParam) => {
  ...
  if (typeof blockParam === "string" && blockParam.startsWith("0x")) {
    if (blockParam.length === 66) {
      const block = await blockchain.getBlock(hexToBytes(
        /** @type {import('@tevm/utils').Hex}*/
        blockParam
      ));
...
var ethGetLogsHandler = (client) => async (params) => {
      ...
      params: [
        {
          ...
          toBlock: toBlock.header.number < forkedBlock.header.number ? numberToHex(toBlock.header.number) : numberToHex(forkedBlock.header.number),
  ...
  const cachedLogs = await receiptsManager.getLogs(
    ...
    toBlock,
    params.filterParams.address !== void 0 ? [createAddress(params.filterParams.address).bytes] : [],
...
var ethGetTransactionReceiptHandler = (client) => async (params) => {
  ...
  return {
    ...
    root: (
      /** @type any*/
      receipt.stateRoot instanceof Uint8Array ? bytesToHex(
        /** @type any*/
        receipt.stateRoot
      ) : void 0
    ),
    status: (
      /** @type any*/
      receipt.status instanceof Uint8Array ? numberToHex(
        /** @type any*/
        receipt.status[0]
      ) : typeof /** @type any*/
      receipt.status === "number" ? numberToHex(
        /** @type any*/
        receipt.status
      ) : void 0
    ),
    logs: await Promise.all(
      receipt.logs.map((log, i) => ({
        ...
        transactionHash: tx !== void 0 ? bytesToHex(tx.hash()) : null
      }))
    )
  };
};
...
var ethNewFilterHandler = (tevmNode) => {
  return async (params) => {
    ...
    const pastLogs = await receiptsManager.getLogs(
      _fromBlock,
      _toBlock,
      address !== void 0 ? [createAddress(address).bytes] : [],
      ...
      logs: pastLogs.map((log) => {
        ...
        return {
          topics: (
            ...
            topics2.map((topic) => bytesToHex(topic))
          ),
          address: bytesToHex(address2),
...
var ethSendTransactionHandler = (client) => async (params) => {
  ...
  if (!tx.isSigned() && impersonatedAccount !== void 0) {
...
var getBalanceHandler = (baseClient) => async ({ address, blockTag = "latest" }) => {
...
var getCodeHandler = (baseClient) => async (params) => {
  ...
  if (await vm.stateManager.hasStateRoot(block.header.stateRoot)) {
    deployedBytecode = vm.stateManager._baseState.stateRoots.get(bytesToHex(block.header.stateRoot))?.[getAddress(params.address)]?.deployedBytecode;
  }
...
var zLoadStateParams = z.object({ state: z.record(zHex, AccountStorage) }).describe("Properties shared across the load state");
...
var ethSendRawTransactionHandler = (client) => async (params) => {
  ...
  try {
    ...
    throw new InvalidTransactionError("Invalid transaction. Unable to parse data", { cause: (
      /** @type {Error}*/
      e
    ) });
  }
