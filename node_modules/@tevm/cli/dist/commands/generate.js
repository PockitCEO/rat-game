...
export const args = zod.tuple([
    zod.enum(['contract', 'test', 'script', 'all']).describe(argument({
        name: 'type',
...
export default function Generate({ args, options }) {
    const [type, name] = args;
    const [isGenerating, setIsGenerating] = useState(false);
    const [currentTask, setCurrentTask] = useState('');
    const [result, setResult] = useState({ success: false, files: [], errors: [] });
    useEffect(() => {
    ...
    if (isGenerating) {
        return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { children: [_jsx(Text, { color: "green", children: _jsx(Spinner, { type: "dots" }) }), _jsxs(Text, { children: [' ', "Generating ", type, " ", name ? `"${name}"` : 'files', options.force ? ' (force)' : ''] })] }), currentTask && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { dimColor: true, children: currentTask }) }))] }));
    }
    return (_jsxs(Box, { flexDirection: "column", children: [result.success ? (_jsxs(_Fragment, { children: [_jsxs(Text, { color: "green", children: ["Successfully generated ", result.files.length, " files:"] }), _jsx(Newline, {}), result.files.map((file, index) => (_jsxs(Text, { children: ["- ", file] }, index)))] })) : (_jsxs(_Fragment, { children: [_jsx(Text, { color: "red", children: "Generation failed with errors:" }), _jsx(Newline, {}), result.errors.map((error, index) => (_jsxs(Text, { color: "red", children: ["- ", error] }, index)))] })), result.files.length > 0 && result.errors.length > 0 && (_jsxs(_Fragment, { children: [_jsx(Newline, {}), _jsx(Text, { color: "yellow", children: "Completed with warnings:" }), _jsx(Newline, {}), result.errors.map((error, index) => (_jsxs(Text, { color: "yellow", children: ["- ", error] }, index)))] }))] }));
}
