...
const parseContracts = (contractsStr) => {
    if (!contractsStr)
        return [];
    try {
...
export default function Multicall({ options }) {
    // Use the action hook
    const actionResult = useAction({
        ...
        optionDescriptions,
        // Create params
        createParams: async (enhancedOptions) => {
    ...
    return (_jsx(CliAction, { ...actionResult, targetName: `${parseContracts(actionResult.options['contracts']).length} contract calls`, successMessage: "Multicall executed successfully!" }));
}
