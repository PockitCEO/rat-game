...
const parseBlockIdentifier = (blockId) => {
    if (!blockId)
        return undefined;
    if (blockId === 'latest' ||
        blockId === 'earliest' ||
...
export default function CreateContractEventFilter({ options }) {
    // Use the action hook
    ...
    return (_jsx(CliAction, { ...actionResult, targetName: `${actionResult.options['eventName'] || 'all'} events on ${actionResult.options['address'] || '0x0000...0000'}`, successMessage: "Contract event filter created successfully!" }));
}
