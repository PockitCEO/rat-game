...
var TxPool = class _TxPool {
  ...
  validateTxGasBump(existingTx, addedTx) {
    ...
    if (addedTx instanceof tx.BlobEIP4844Transaction && existingTx instanceof tx.BlobEIP4844Transaction) {
  ...
  async validate(tx, isLocalTransaction = false, requireSignature = true, skipBalance = false) {
    ...
    if (inPool) {
      ...
      if (existingTxn) {
        if (utils.equalsBytes(existingTxn.tx.hash(), tx.hash())) {
          throw new Error(`${utils.bytesToHex(tx.hash())}: this transaction is already in the TxPool`);
        }
  ...
  async addUnverified(tx) {
    const hash = utils.bytesToUnprefixedHex(tx.hash());
    ...
    try {
      ...
      this.txsInPool++;
      this.fireEvent("txadded", utils.bytesToHex(tx.hash()));
      return { error: null, hash: utils.bytesToHex(tx.hash()) };
    } catch (e) {
      this.handled.set(hash, { address, added, error: e });
      return { error: e.message, hash: utils.bytesToHex(tx.hash()) };
    }
  }
  /**
  ...
  getByHash(txHashes) {
    ...
    for (const txHash of txHashes) {
      const txHashStr = utils.bytesToUnprefixedHex(txHash);
      const handled = this.handled.get(txHashStr);
      if (!handled) continue;
      const inPool = this.pool.get(handled.address)?.filter((poolObj) => poolObj.hash === txHashStr);
      if (inPool && inPool.length === 1) {
  ...
  removeNewBlockTxs(newBlocks) {
    ...
    for (const block of newBlocks) {
      for (const tx of block.transactions) {
        const txHash = utils.bytesToUnprefixedHex(tx.hash());
   ...
   */
  normalizedGasPrice(tx$1, baseFee) {
    const supports1559 = tx$1.supports(tx.Capability.EIP1559FeeMarket);
    if (typeof baseFee === "bigint" && baseFee !== 0n) {
      if (supports1559) {
        return tx$1.maxPriorityFeePerGas;
      }
      return tx$1.gasPrice - baseFee;
    }
    if (supports1559) {
      return tx$1.maxFeePerGas;
    }
    return tx$1.gasPrice;
  }
  /**
   ...
   */
  txGasPrice(tx$1) {
    if ("isImpersonated" in tx$1 && tx$1.isImpersonated) {
      return {
        maxFee: tx$1.maxFeePerGas,
        tip: tx$1.maxPriorityFeePerGas
      };
    }
    if (tx.isLegacyTx(tx$1)) {
      return {
        maxFee: tx$1.gasPrice,
        tip: tx$1.gasPrice
      };
    }
    if (tx.isAccessListEIP2930Tx(tx$1)) {
      return {
        maxFee: tx$1.gasPrice,
        tip: tx$1.gasPrice
      };
    }
    if (tx.isFeeMarketEIP1559Tx(tx$1) || tx.isBlobEIP4844Tx(tx$1)) {
      return {
        maxFee: tx$1.maxFeePerGas,
        tip: tx$1.maxPriorityFeePerGas
      };
    }
    throw new Error(`tx of type ${tx$1.type} unknown`);
  }
  ...
  async onChainReorganization(removedBlocks, addedBlocks) {
    for (const block of removedBlocks) {
      for (const tx of block.transactions) {
        const txHash = utils.bytesToHex(tx.hash());
  ...
  async txsByPriceAndNonce({ baseFee, allowedBlobs } = {}) {
    ...
    const byPrice = new Heap__default.default({
    ...
    while (byPrice.length > 0) {
      ...
      if (!(best instanceof tx.BlobEIP4844Transaction) || allowedBlobs === void 0 || (best.blobs ?? []).length + blobsCount <= allowedBlobs) {
        if (accTxs.length > 0) {
          if (!accTxs[0]) {
            throw new Error("Expected accTxs to be defined");
          }
        ...
        txs.push(best);
        if (best instanceof tx.BlobEIP4844Transaction) {
