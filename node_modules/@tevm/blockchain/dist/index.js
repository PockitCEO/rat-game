...
import { hexToBytes, bytesToHex, withRetry } from 'viem';
import { UnknownBlockError, InvalidBlockError, InternalError } from '@tevm/errors';
import { createJsonRpcFetcher } from '@tevm/jsonrpc';
import { numberToHex, bytesToHex as bytesToHex$1, bytesToBigInt, isHex, hexToBytes as hexToBytes$1, hexToNumber } from '@tevm/utils';
...
var getBlock = (baseChain) => async (blockId) => {
  const block = (() => {
    ...
    if (blockId instanceof Uint8Array) {
      baseChain.logger.debug({ blockId: bytesToHex$1(blockId) }, "getting block by hash or hex encoded number");
      return baseChain.blocks.get(bytesToHex$1(blockId)) ?? baseChain.blocksByNumber.get(bytesToBigInt(blockId));
    }
  ...
  if (!baseChain.options.fork?.transport) {
    throw new UnknownBlockError(
      blockId instanceof Uint8Array ? `Block with hash ${bytesToHex$1(blockId)} does not exist` : `Block number ${blockId} does not exist`
    );
  }
  ...
  const [fetchedBlock, jsonRpcFetchedBlock] = await getBlockFromRpc(
    baseChain,
    {
      ...
      blockTag: blockId instanceof Uint8Array ? bytesToHex$1(blockId) : BigInt(blockId)
    },
    baseChain.common
  );
...
var setIteratorHead = (baseChain) => (tag, headHash) => {
  baseChain.blocksByTag.set(
    /** @type {import('viem').BlockTag}*/
    tag,
    baseChain.blocks.get(bytesToHex(headHash))
  );
  return Promise.resolve();
};
...
var getBlockByTag = (baseChain) => async (blockId) => {
  ...
  if (isHex(blockId)) {
    return blockId.length === 66 ? _getBlock(hexToBytes$1(blockId)) : _getBlock(hexToNumber(blockId));
  }
...
export { createBaseChain, createChain, deepCopy, delBlock, getBlock, getBlockFromRpc, getCanonicalHeadBlock, getIteratorHead, isTevmBlockTag, putBlock, setIteratorHead, shallowCopy, validateHeader };
