import { equalsBytes, hexToBytes, stringToHex, numberToHex, bytesToNumber, bytesToBigInt, Bloom, toHex, concatBytes } from '@tevm/utils';
        ...
        logs = logs.filter((l) => {
          for (const [i2, topic] of topics.entries()) {
            if (Array.isArray(topic)) {
              if (!topic.find((t) => equalsBytes(t, l.log[1][i2]))) return false;
            } else if (!topic) ; else {
              if (!equalsBytes(topic, l.log[1][i2])) return false;
            }
            return true;
          }
          return false;
    ...
    switch (type) {
        ...
        if (conversion === 0 /* Encode */) {
          return Rlp.encode(
            value.map((r) => [
              ...
              hexToBytes(numberToHex(r.status)),
      ...
      case 2 /* TxHash */: {
        if (conversion === 0 /* Encode */) {
          ...
          return Rlp.encode([blockHash2, hexToBytes(numberToHex(txIndex2))]);
        }
        const [blockHash, txIndex] = Rlp.decode(value);
        return [blockHash, bytesToNumber(txIndex)];
      }
      default:
...
export { ReceiptsManager, createMapDb };
