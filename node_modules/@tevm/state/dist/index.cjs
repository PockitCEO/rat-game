...
var generateCanonicalGenesis = (baseState) => async (state) => {
  ...
  try {
    for (const [k, v] of Object.entries(
      ...
      baseState.caches.accounts?.put(address$1, account);
      if (deployedBytecode) {
...
var modifyAccountFields = (baseState) => async (address, accountFields) => {
  const account = await getAccount(baseState)(address) ?? utils.createAccount({});
...
var putContractCode = (baseState) => async (address, value) => {
  ...
  await putAccount(baseState)(address, utils.createAccount(accountData));
