...
var ClRequest = class {
  ...
  constructor(type, bytes) {
    if (type === void 0) throw new errors.InternalError("request type is required");
    this.type = type;
    this.bytes = bytes;
  }
  serialize() {
    return utils.concatBytes(Uint8Array.from([this.type]), this.bytes);
  }
};
...
function executionPayloadFromBeaconPayload(payload) {
  const executionPayload = {
    ...
    blockNumber: utils.numberToHex(BigInt(payload.block_number)),
    gasLimit: utils.numberToHex(BigInt(payload.gas_limit)),
    gasUsed: utils.numberToHex(BigInt(payload.gas_used)),
    timestamp: utils.numberToHex(BigInt(payload.timestamp)),
    extraData: payload.extra_data,
    baseFeePerGas: utils.numberToHex(BigInt(payload.base_fee_per_gas)),
  ...
  if (payload.withdrawals !== void 0 && payload.withdrawals !== null) {
    executionPayload.withdrawals = payload.withdrawals.map((wd) => ({
      index: utils.numberToHex(BigInt(wd.index)),
      validatorIndex: utils.numberToHex(BigInt(wd.validator_index)),
      address: wd.address,
      amount: utils.numberToHex(BigInt(wd.amount))
    }));
  }
  if (payload.blob_gas_used !== void 0 && payload.blob_gas_used !== null) {
    executionPayload.blobGasUsed = utils.numberToHex(BigInt(payload.blob_gas_used));
  }
  if (payload.excess_blob_gas !== void 0 && payload.excess_blob_gas !== null) {
    executionPayload.excessBlobGas = utils.numberToHex(BigInt(payload.excess_blob_gas));
  }
...
function createZeroAddress() {
  return new utils.EthjsAddress(new Uint8Array(20));
}
function createAddressFromPublicKey(publicKey) {
  const addressBytes = utils.keccak256(publicKey, "bytes").slice(-20);
  return new utils.EthjsAddress(addressBytes);
}
function safeToType(input, outputType) {
  ...
  if (typeof input === "string" && outputType === 2) {
    const bytes = utils.toBytes(input);
    return utils.toType(bytes, outputType);
  }
  return utils.toType(input, outputType);
}
...
var bigIntToUnpaddedBytes = (n) => {
  return utils.toBytes(n).slice(2);
};
var BlockHeader = class _BlockHeader {
  ...
  static fromRLPSerializedHeader(serializedHeaderData, opts) {
    const values = rlp.Rlp.decode(serializedHeaderData);
  ...
  static fromValuesArray(values, opts) {
    ...
    if (header.common.ethjsCommon.isActivatedEIP(1559) && baseFeePerGas === void 0) {
      const eip1559ActivationBlock = utils.toBytes(header.common.ethjsCommon.eipBlock(1559));
      if (eip1559ActivationBlock !== void 0 && utils.equalsBytes(eip1559ActivationBlock, number)) {
  ...
  constructor(headerData, opts) {
    if (opts.common) {
      ...
      this.common = common.tevmDefault.copy();
    }
    this.keccakFunction = this.common.ethjsCommon.customCrypto.keccak256 ?? ((value) => utils.keccak256(value, "bytes"));
    const skipValidateConsensusFormat = opts.skipConsensusFormatValidation ?? false;
    const defaults = {
      parentHash: zeros(32),
      uncleHash: utils.KECCAK256_RLP_ARRAY,
      coinbase: createZeroAddress(),
      stateRoot: zeros(32),
      transactionsTrie: utils.KECCAK256_RLP,
      receiptTrie: utils.KECCAK256_RLP,
      logsBloom: zeros(256),
      difficulty: 0n,
      number: 0n,
    ...
    const coinbase = new utils.EthjsAddress(safeToType(headerData.coinbase ?? defaults.coinbase, 2));
    ...
    const hardforkDefaults = {
      ...
      withdrawalsRoot: this.common.ethjsCommon.isActivatedEIP(4895) ? utils.KECCAK256_RLP : void 0,
      ...
      requestsRoot: this.common.ethjsCommon.isActivatedEIP(7685) ? utils.KECCAK256_RLP : void 0
    };
    ...
    if (opts.calcDifficultyFromHeader && this.common.ethjsCommon.consensusAlgorithm() === common.ConsensusAlgorithm.Ethash) {
    ...
    if (opts.cliqueSigner) {
      ...
      if (this.extraData.length < minExtraDataLength) {
        ...
        this.extraData = utils.concatBytes(this.extraData, new Uint8Array(remainingLength));
      }
  ...
  _consensusFormatValidation() {
    ...
    if (this.common.ethjsCommon.consensusAlgorithm() === common.ConsensusAlgorithm.Ethash) {
    ...
    if (this.common.ethjsCommon.consensusAlgorithm() === common.ConsensusAlgorithm.Clique) {
      ...
      if (!utils.equalsBytes(this.mixHash, new Uint8Array(32))) {
        const msg = this._errorMsg(`mixHash must be filled with zeros, received ${this.mixHash}`);
        throw new Error(msg);
      }
    }
    if (this.common.ethjsCommon.consensusType() === common.ConsensusType.ProofOfStake) {
      let error = false;
      let errorMsg = "";
      if (!utils.equalsBytes(uncleHash, utils.KECCAK256_RLP_ARRAY)) {
        errorMsg += `, uncleHash: ${utils.bytesToHex(uncleHash)} (expected: ${utils.bytesToHex(utils.KECCAK256_RLP_ARRAY)})`;
        error = true;
      }
      if (number !== 0n) {
        if (difficulty !== 0n) {
        ...
        if (extraData.length > 32) {
          errorMsg += `, extraData: ${utils.bytesToHex(extraData)} (cannot exceed 32 bytes length, received ${extraData.length} bytes)`;
          error = true;
        }
        if (!utils.equalsBytes(nonce, zeros(8))) {
          errorMsg += `, nonce: ${utils.bytesToHex(nonce)} (expected: ${utils.bytesToHex(zeros(8))})`;
          error = true;
        }
      }
      if (error) {
  ...
  hash() {
    if (Object.isFrozen(this)) {
      if (!this.cache.hash) {
        this.cache.hash = this.keccakFunction(rlp.Rlp.encode(this.raw()));
      }
      return this.cache.hash;
    }
    return this.keccakFunction(rlp.Rlp.encode(this.raw()));
  }
  /**
  ...
  ethashCanonicalDifficulty(parentBlockHeader) {
    if (this.common.ethjsCommon.consensusType() !== common.ConsensusType.ProofOfWork) {
    ...
    if (this.common.ethjsCommon.gteHardfork("byzantium") === true) {
      const uncleAddend = utils.equalsBytes(parentBlockHeader.uncleHash, utils.KECCAK256_RLP_ARRAY) ? 1 : 2;
  ...
  cliqueSigHash() {
    ...
    return this.keccakFunction(rlp.Rlp.encode(raw));
  }
  /**
  ...
  cliqueSealBlock(privateKey) {
    ...
    const rBytes = utils.toBytes(signature.r);
    const sBytes = utils.toBytes(signature.s);
    const signatureB = utils.concatBytes(rBytes, sBytes, vBytes);
    const extraDataWithoutSeal = this.extraData.subarray(0, this.extraData.length - CLIQUE_EXTRA_SEAL);
    const extraData = utils.concatBytes(extraDataWithoutSeal, signatureB);
    return extraData;
  }
  /**
  ...
  cliqueEpochTransitionSigners() {
    ...
    return signerList.map((buf) => new utils.EthjsAddress(buf));
  }
  /**
  ...
  cliqueSigner() {
    ...
    if (extraSeal.length === 0 || utils.equalsBytes(extraSeal, new Uint8Array(65))) {
      return createZeroAddress();
    }
    const r = extraSeal.subarray(0, 32);
    const s = extraSeal.subarray(32, 64);
    const v = utils.bytesToBigInt(extraSeal.subarray(64, 65)) + 27n;
    const pubKey = utils.ecrecover(this.cliqueSigHash(), v, r, s);
   ...
   */
  serialize() {
    return rlp.Rlp.encode(this.raw());
  }
  /**
   * Returns the block header in JSON format.
   */
  toJSON() {
    const withdrawalAttr = this.withdrawalsRoot ? { withdrawalsRoot: utils.bytesToHex(this.withdrawalsRoot) } : {};
    const jsonDict = {
      parentHash: utils.bytesToHex(this.parentHash),
      uncleHash: utils.bytesToHex(this.uncleHash),
      coinbase: this.coinbase.toString(),
      stateRoot: utils.bytesToHex(this.stateRoot),
      transactionsTrie: utils.bytesToHex(this.transactionsTrie),
      ...withdrawalAttr,
      receiptTrie: utils.bytesToHex(this.receiptTrie),
      logsBloom: utils.bytesToHex(this.logsBloom),
      difficulty: utils.numberToHex(this.difficulty),
      number: utils.numberToHex(this.number),
      gasLimit: utils.numberToHex(this.gasLimit),
      gasUsed: utils.numberToHex(this.gasUsed),
      timestamp: utils.numberToHex(this.timestamp),
      extraData: utils.bytesToHex(this.extraData),
      mixHash: utils.bytesToHex(this.mixHash),
      nonce: utils.bytesToHex(this.nonce)
    };
    if (this.common.ethjsCommon.isActivatedEIP(1559)) {
      jsonDict.baseFeePerGas = utils.numberToHex(this.baseFeePerGas);
    }
    if (this.common.ethjsCommon.isActivatedEIP(4844)) {
      jsonDict.blobGasUsed = utils.numberToHex(this.blobGasUsed);
      jsonDict.excessBlobGas = utils.numberToHex(this.excessBlobGas);
    }
    if (this.common.ethjsCommon.isActivatedEIP(4788)) {
      jsonDict.parentBeaconBlockRoot = utils.bytesToHex(this.parentBeaconBlockRoot);
    }
    if (this.common.ethjsCommon.isActivatedEIP(7685)) {
      jsonDict.requestsRoot = utils.bytesToHex(this.requestsRoot);
    }
    return jsonDict;
  }
  /**
  ...
  _validateDAOExtraData() {
    ...
    const DAO_ExtraData = utils.hexToBytes("0x64616f2d686172642d666f726b");
    const DAO_ForceExtraDataRange = BigInt(9);
    const drift = this.number - DAOActivationBlock;
    if (drift <= DAO_ForceExtraDataRange && !utils.equalsBytes(this.extraData, DAO_ExtraData)) {
      const msg = this._errorMsg(
        `extraData should be 'dao-hard-fork', got ${utils.bytesToUtf8(this.extraData)} (hex: ${utils.bytesToHex(this.extraData)})`
      );
      throw new Error(msg);
    }
  }
  ...
  errorStr() {
    let hash = "";
    try {
      hash = utils.bytesToHex(this.hash());
    } catch (_e) {
      hash = "error";
    }
    let hf = "";
...
var Block = class _Block {
  ...
  static async genWithdrawalsTrieRoot(wts, emptyTrie) {
    const trie$1 = emptyTrie ?? new trie.Trie();
    for (const [i, wt] of wts.entries()) {
      await trie$1.put(rlp.Rlp.encode(i), rlp.Rlp.encode(wt.raw()));
    }
    return trie$1.root();
  }
  /**
  ...
  static async genRequestsTrieRoot(requests, emptyTrie) {
    ...
    for (const [i, req] of requests.entries()) {
      await trie$1.put(rlp.Rlp.encode(i), req.serialize());
    }
    return trie$1.root();
  }
  /**
  ...
  static fromBlockData(blockData, opts) {
    ...
    const withdrawals = withdrawalsData?.map(utils.createWithdrawal);
  ...
  static fromRLPSerializedBlock(serialized, opts) {
    const values = rlp.Rlp.decode(Uint8Array.from(serialized));
    if (!Array.isArray(values)) {
  ...
  static fromValuesArray(values, opts) {
    ...
    for (const txData of txsData ?? []) {
      transactions.push(
        tx.createTxFromBlockBodyData(txData, {
          ...opts,
          // Use header common in case of setHardfork being activated
    ...
    const withdrawals = withdrawalBytes?.map(([index, validatorIndex, address, amount]) => ({
      ...
      address,
      amount
    }))?.map((w) => utils.createWithdrawal(w));
    let requests = [];
    ...
    if (header.common.ethjsCommon.isActivatedEIP(6800)) {
      if (executionWitnessBytes !== void 0) {
        executionWitness = JSON.parse(utils.bytesToUtf8(rlp.Rlp.decode(executionWitnessBytes)));
  ...
  static async fromExecutionPayload(payload, opts) {
    ...
    for (const [index, serializedTx] of transactions.entries()) {
      try {
        const tx$1 = tx.createTxFromRLP(utils.hexToBytes(serializedTx), {
    ...
    const transactionsTrie = await _Block.genTransactionsTrieRoot(txs, new trie.Trie({ common: opts?.common.ethjsCommon }));
    const withdrawals = withdrawalsData?.map((wData) => utils.createWithdrawal(wData));
    const withdrawalsRoot = withdrawals ? await _Block.genWithdrawalsTrieRoot(withdrawals, new trie.Trie({ common: opts?.common.ethjsCommon })) : void 0;
    const header = {
      ...payload,
      number,
      receiptTrie,
    ...
    if (!utils.equalsBytes(block.hash(), utils.hexToBytes(payload.blockHash))) {
      const validationError = `Invalid blockHash, expected: ${payload.blockHash}, received: ${utils.bytesToHex(block.hash())}`;
  ...
  constructor(opts, header, transactions = [], uncleHeaders = [], withdrawals, requests, executionWitness) {
    ...
    this.keccakFunction = this.common.ethjsCommon.customCrypto.keccak256 ?? ((item) => utils.keccak256(item, "bytes"));
    ...
    if (uncleHeaders.length > 0) {
      this.validateUncles();
      if (this.common.ethjsCommon.consensusType() === common.ConsensusType.ProofOfAuthority) {
  ...
  raw() {
    const bytesArray = [
      ...
      this.transactions.map(
        (tx$1) => tx$1.supports(tx.Capability.EIP2718TypedTransaction) ? tx$1.serialize() : tx$1.raw()
      ),
    ...
    if (this.executionWitness !== void 0 && this.executionWitness !== null) {
      const executionWitnessBytes = rlp.Rlp.encode(JSON.stringify(this.executionWitness));
   ...
   */
  async genTxTrie() {
    return _Block.genTransactionsTrieRoot(this.transactions, new trie.Trie({ common: this.common.ethjsCommon }));
  }
  /**
  ...
  async transactionsTrieIsValid() {
    ...
    if (this.transactions.length === 0) {
      result = utils.equalsBytes(this.header.transactionsTrie, utils.KECCAK256_RLP);
      return result;
    }
    if (this.cache.txTrieRoot === void 0) {
      this.cache.txTrieRoot = await this.genTxTrie();
    }
    result = utils.equalsBytes(this.cache.txTrieRoot, this.header.transactionsTrie);
    return result;
  }
  async requestsTrieIsValid() {
    ...
    if (this.requests?.length === 0) {
      result = utils.equalsBytes(this.header.requestsRoot, utils.KECCAK256_RLP);
      return result;
    }
    if (this.cache.requestsRoot === void 0) {
      this.cache.requestsRoot = await _Block.genRequestsTrieRoot(this.requests ?? []);
    }
    result = utils.equalsBytes(this.cache.requestsRoot, this.header.requestsRoot);
    return result;
  }
  /**
  ...
  getTransactionsValidationErrors() {
    ...
    const blobGasPerBlob = this.common.ethjsCommon.param("blobGasPerBlob");
    for (let [i, tx$1] of this.transactions.entries()) {
      const errs = tx$1.getValidationErrors();
      if (this.common.ethjsCommon.isActivatedEIP(1559)) {
        if (tx$1.supports(tx.Capability.EIP1559FeeMarket)) {
          tx$1 = tx$1;
          if (tx$1.maxFeePerGas < this.header.baseFeePerGas) {
            errs.push("tx unable to pay base fee (EIP-1559 tx)");
          }
        } else {
          tx$1 = tx$1;
          if (tx$1.gasPrice < this.header.baseFeePerGas) {
      ...
      if (this.common.ethjsCommon.isActivatedEIP(4844)) {
        if (tx$1 instanceof tx.BlobEIP4844Transaction) {
          blobGasUsed += BigInt(tx$1.numBlobs()) * blobGasPerBlob;
  ...
  validateBlobTransactions(parentHeader) {
    if (this.common.ethjsCommon.isActivatedEIP(4844)) {
      ...
      for (const tx$1 of this.transactions) {
        if (tx$1 instanceof tx.BlobEIP4844Transaction) {
          blobGasPrice = blobGasPrice ?? this.header.getBlobGasPrice();
          if (tx$1.maxFeePerBlobGas < blobGasPrice) {
            throw new Error(
              `blob transaction maxFeePerBlobGas ${tx$1.maxFeePerBlobGas} < than block blob gas price ${blobGasPrice} - ${this.errorStr()}`
            );
          }
          blobGasUsed += BigInt(tx$1.blobVersionedHashes.length) * blobGasPerBlob;
          if (blobGasUsed > blobGasLimit) {
            throw new Error(
              `tx causes total blob gas of ${blobGasUsed} to exceed maximum blob gas per block of ${blobGasLimit}`
            );
  ...
  uncleHashIsValid() {
    if (this.uncleHeaders.length === 0) {
      return utils.equalsBytes(utils.KECCAK256_RLP_ARRAY, this.header.uncleHash);
    }
    const uncles = this.uncleHeaders.map((uh) => uh.raw());
    const raw = rlp.Rlp.encode(uncles);
    return utils.equalsBytes(this.keccakFunction(raw), this.header.uncleHash);
  }
  /**
  ...
  async withdrawalsTrieIsValid() {
    ...
    if (this.withdrawals?.length === 0) {
      result = utils.equalsBytes(this.header.withdrawalsRoot, utils.KECCAK256_RLP);
      return result;
    }
    if (this.cache.withdrawalsTrieRoot === void 0) {
      this.cache.withdrawalsTrieRoot = await _Block.genWithdrawalsTrieRoot(
        this.withdrawals ?? [],
        new trie.Trie({ common: this.common.ethjsCommon })
      );
    }
    result = utils.equalsBytes(this.cache.withdrawalsTrieRoot, this.header.withdrawalsRoot);
    return result;
  }
  /**
  ...
  validateUncles() {
    ...
    const uncleHashes = this.uncleHeaders.map((header) => utils.bytesToHex(header.hash()));
  ...
  toJSON() {
    ...
    return {
      ...
      ...withdrawalsAttr,
      requests: this.requests?.map((req) => utils.bytesToHex(req.serialize()))
    };
  }
  toExecutionPayload() {
    const blockJson = this.toJSON();
    const header = blockJson.header;
    const transactions = this.transactions.map((tx) => utils.bytesToHex(tx.serialize())) ?? [];
    ...
    const executionPayload = {
      ...
      blockHash: utils.bytesToHex(this.hash()),
...
function normalizeTxParams(_txParams) {
  ...
  txParams.gasLimit = utils.toType(txParams.gasLimit ?? txParams.gas, utils.TypeOutput.BigInt);
  ...
  txParams.to = txParams.to !== null && txParams.to !== void 0 ? utils.setLengthLeft(utils.toBytes(txParams.to), 20) : null;
  txParams.v = utils.toType(txParams.v, utils.TypeOutput.BigInt);
  return txParams;
}
function blockFromRpc(blockParams, options, uncles = []) {
  ...
  for (const _txParams of blockParams.transactions ?? []) {
    ...
    try {
      const tx$1 = tx.TransactionFactory(txParams, opts);
      transactions.push(tx$1);
    } catch (e) {
