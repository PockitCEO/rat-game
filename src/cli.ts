...
/**
 * Rat Game - Terminal UI (TypeScript)
...
const render = (): void => {
  ...
  console.log(
    chalk.gray(
      `Block: ${state.blockNumber} | Grid: ${state.gridSize}x${state.gridSize} | Price: ${state.marketPrice} wei\n`
    )
  );
  ...
  if (state.agents.length > 0) {
    ...
    state.agents.forEach((agent) => {
      console.log(
        `  ${chalk.yellow(agent.id.substring(0, 12))} @ (${agent.x}, ${agent.y}) | ` +
  ...
  if (cheeses.length > 0) {
    ...
    cheeses.forEach((cheese) => {
      console.log(
        `  🧀 ${chalk.green(cheese.id.substring(0, 12))} @ (${cheese.x}, ${cheese.y})`
      );
    });
  } else {
  ...
  try {
    switch (action) {
        ...
        const { x, y } = await inquirer.prompt([
          {
            type: 'number',
            name: 'x',
            message: 'X coordinate (0-99):',
            default: 0,
          },
          {
            type: 'number',
            name: 'y',
            message: 'Y coordinate (0-99):',
            default: 0,
          },
        ]);
        const agentId = `rat_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
      ...
      case 'move': {
        ...
        console.log(
          moved ? chalk.green('✓ Moved') : chalk.yellow('✗ Could not move')
        );
        await inquirer.prompt([
          { type: 'input', name: 'x', message: 'Press enter to continue...' },
        ]);
        break;
      }
      ...
      case 'eat': {
        const agents = game.getAgents();
        if (agents.length === 0) {
          console.log(chalk.red('No agents'));
          await inquirer.prompt([
            {
              type: 'input',
              name: 'x',
        ...
        const { agentId } = await inquirer.prompt([
          {
            ...
            message: 'Select agent:',
            choices: agents.map((a) => ({
              name: `${a.id} (${a.cheeseInventory} cheese)`,
              value: a.id,
            })),
          },
        ]);
  ...
  const { gridSize } = await inquirer.prompt([
    {
      type: 'number',
      name: 'gridSize',
      message: 'Grid size (default 100):',
      default: 100,
    },
  ]);

  const g = new RatGame(gridSize);
  g.init();

  // Replace global game instance
